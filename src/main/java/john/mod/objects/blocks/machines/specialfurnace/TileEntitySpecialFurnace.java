...
public class TileEntitySpecialFurnace extends TileEntity implements IInventory, ITickable
{
	...
	{
		return this.hasCustomName() ? this.customName : "container.special_furnace"; //Ternary: Wenn custom Name vorhanden: ? sonst :
	}
	...
	public void update()
	{
		...
		if(!this.world.isRemote)
		{
			...
			{
				flag1 = true;
				BlockSpecialFurnace.setState(this.isBurning(), this.world, this.pos);
			}
		}
	...
	private boolean canSmelt()
	{
		...
		{
			ItemStack result = SpecialFurnaceRecipes.getInstance().getSpecialResult((ItemStack)this.inventory.get(0), (ItemStack)this.inventory.get(1));
			if(result.isEmpty()) return false;
			else
			{
				ItemStack output = (ItemStack)this.inventory.get(3); //Feld 3
				if(output.isEmpty()) return true;
				if(!output.isItemEqual(result)) return false;
				int res = output.getCount() + result.getCount();							// Prüft ob nach Vorgang genug Platz da wäre
	...
	public void smeltItem()
	{
		if(this.canSmelt())
		{
			...
			ItemStack result = SpecialFurnaceRecipes.getInstance().getSpecialResult(input1, input2);
